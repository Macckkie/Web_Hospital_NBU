...
try {
    ...
    if ($role === 'admin') {
        ...
        $logsStmt = $pdo->query("SELECT a.*, u.username FROM activity_logs a LEFT JOIN users u ON a.user_id = u.id ORDER BY a.created_at DESC LIMIT 15");
        $data['activity_logs'] = $logsStmt->fetchAll();
